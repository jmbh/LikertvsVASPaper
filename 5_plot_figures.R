# jonashaslbeck@gmail.com; March 3rd, 2025
...
plotFig <- function(data,
  ...
  if(legend) legend(legendLoc, legend=c("Visual Analogue Scale",
                                        "Likert Scale",
                                        "VAS mapped to Likert")[include],
                    text.col=cols[c(2,1,3)][include], bty='n', cex=1, pch=c(19,15,17), col=cols[c(2,1,3)])
...
plotFig_extVal <- function(Model, 
  ...
  # Legend
  if(legend) legend(6, -.5, legend=c("Visual Analogue Scale",
                                     "Likert Scale"),
                    text.col=cols[c(2,1,3)], bty='n', cex=1, pch=c(19,15,17), col=cols[c(2,1,3)])
...
# We still use this for a figure in the appendix
...
plotFig_extVal_Data <- function(data1,
  ...
  if(add == FALSE) {
    ...
    if(legend) legend("bottomright", legend=c("Visual Analogue Scale",
                                              "Likert Scale"),
                      text.col=cols[c(2,1,3)], bty='n', cex=1, pch=c(19,15,17), col=cols[c(2,1,3)])
...
PlotParsEMM <- function(Model="Mean", ylim=c(-.1, .6)) {
  ...
  axis(1, 1, labels="VAS", col.axis=cols[2], las=2)
  axis(1, 2, labels="Likert", col.axis=cols[1], las=2)
  ...
  # Plot VAS:
  points(1, pars$VAS[1], pch=15, cex=1.2, col=cols[2])
  text(1, pars$VAS[1]+vdis, round(pars$VAS[1],2), col=cols[2])
  segments(1, pars$VAS[2], 1, pars$VAS[3], lwd=3, col=cols[2])
  # Plot Likert:
  points(2, pars$Likert[1], pch=19, cex=1.2, col=cols[1])
  text(2, pars$Likert[1]+vdis, round(pars$Likert[1],2), col=cols[1])
  segments(2, pars$Likert[2], 2, pars$Likert[3], lwd=3, col=cols[1])
  # Plot Diff:
...
PlotParsEMM_VAS2Lik <- function(Model="MappingMean", 
  ...
  cex.axis=0.8
  axis(1, 1, labels="VAS", col.axis=cols[2], las=2, cex.axis=cex.axis)
  axis(1, 2, labels="Likert", col.axis=cols[1], las=2, cex.axis=cex.axis)
  ...
  vdis <- (ylim[2]-ylim[1]) * 0.2
  # Plot VAS:
  points(2, pars$VAS[pars$Estimate=="Median"], pch=19, cex=1.2, col=cols[1])
  text(2, pars$VAS[pars$Estimate=="Median"]+vdis, round(pars$VAS[pars$Estimate=="Median"],2), col=cols[1])
  segments(2, pars$VAS[pars$Estimate=="Lower"], 2, pars$VAS[pars$Estimate=="Upper"], lwd=3, col=cols[1])
  # Plot Likert:
  points(1, pars$Likert[pars$Estimate=="Median"], pch=19, cex=1.2, col=cols[2])
  text(1, pars$Likert[pars$Estimate=="Median"]+vdis, round(pars$Likert[pars$Estimate=="Median"],2), col=cols[2])
  segments(1, pars$Likert[pars$Estimate=="Lower"], 1, pars$Likert[pars$Estimate=="Upper"], lwd=3, col=cols[2])
  # Plot Diff:
...
EMM_EMV[EMM_EMV$Model=="Mean" & EMM_EMV$Component=="Mean", ]
EMM_EMV[EMM_EMV$Model=="Mean" & EMM_EMV$Component=="Var", ]
...
pdf(paste0("Figures/SanityCheck_Hist_Likert.pdf"), width = 7, height = 5)
for(i in 1:length(u_subj_Lik)) {
  ...
  for(j in 4:17) {
    par(mar=c(4,3,2,1))
    tb <- rep(NA, 7)
    for(s in 1:7) tb[s] <- sum(data_Lik_i[, j] == s, na.rm = TRUE)
    names(tb) <- 1:7
    bp <- barplot(tb, main="", xlab="", ylab="", ylim=c(0,84),  names.arg=FALSE, axes=FALSE)
    axis(1, labels=1:7, at=bp)
    axis(2, las=2)
    title(main=paste0(i, "; Item = ", colnames(data_Lik)[j]), font.main=1, cex.main=0.7)
  }
  # Empty
  plot.new()
  plot.window(xlim=c(1,84), ylim=c(1,7))
...
pdf(paste0("Figures/SanityCheck_Hist_VAS.pdf"), width = 7, height = 5)
for(i in 1:length(u_subj_VAS)) {
  
  # Subset data
  data_VAS_i <- data_VAS[data_VAS$Name == u_subj_VAS[i], ] # get data from one person
  par(mfrow=c(3,5))
  for(j in 4:17) {
    par(mar=c(4,3,2,1))
    # tb <- rep(NA, 7)
    ...
    # names(tb) <- 1:7
    # bp <- barplot(tb, main="", xlab="", ylab="", ylim=c(0,84),  names.arg=FALSE, axes=FALSE)
    hist(data_VAS_i[, j], axes=FALSE, breaks=seq(0,100, length=30), main="", xlab="", ylim=c(0, 25))
    axis(1)
    axis(2, las=2)
    title(main=paste0(i, "; Item = ", colnames(data_Lik)[j]), font.main=1, cex.main=0.7)
  }
...
# ----------------------------------------------------
# -------- Correlation Matrices of All Persons -------
# ----------------------------------------------------

# Plot correlation matrix for every person separately

N_vas <- dim(l_cors_av$a_cors_VAS)[3]
N_lik <- dim(l_cors_av$a_cors_Lik)[3]

pdf("Figures/Heatplot_WithinPersonCors.pdf", width = 7, height = 6)

for(i in 1:N_vas) {
  PlotHeat(data=l_cors_av$a_cors_VAS[, , i], 
           names=names[4:17], cex=0.5, maxcor=.8,
           title=paste0("Subject ", i, ": VAS"))
}
for(i in 1:N_lik) {
  PlotHeat(data=l_cors_av$a_cors_Lik[, , i], 
           names=names[4:17], cex=0.5, maxcor=.8,
           title=paste0("Subject ", i, ": Likert"))
}

dev.off()

# Person 35: VAS
u_ptp_VAS <- unique(data_VAS$Name)
check_data <- data_VAS[data_VAS$Name==u_ptp_VAS[35], ]
head(check_data)
dim(check_data)

plot(check_data$Happy, type="l")
lines(check_data$Sad, col="red")
